...
fn eval_stmt(statement: &Stmt, arena: &mut PyArena) -> Result<StatementOperation, PyException> {
    Ok(match statement {
        ...
        Stmt::AugAssign(stmt_aug_assign) => eval_stmt_aug_assign(&stmt_aug_assign.target, &stmt_aug_assign.op, &stmt_aug_assign.value, stmt_aug_assign.range, arena)?,
        // not implemented
...
fn op_to_magic_method(op: &Operator) -> PyMagicMethod {
    match op {
        Operator::Add => PyMagicMethod::Add {right: false},
        Operator::Sub => PyMagicMethod::Sub {right: false},
        Operator::Mult => PyMagicMethod::Mul {right: false},
        Operator::MatMult => todo!(),
        Operator::Div => PyMagicMethod::TrueDiv {right: false},
        Operator::Mod => todo!(),
        Operator::Pow => PyMagicMethod::Pow {right: false},
...
fn eval_stmt_aug_assign(target: &Box<Expr>, op: &Operator, value: &Box<Expr>, _range: TextRange, arena: &mut PyArena) -> Result<StatementOperation, PyException> {
    let target_name = expect_name(&target, arena)?;
    let target_value = eval_expr(target, arena)?;
    let value_value = eval_expr(value, arena)?;

    let op_magic_method = op_to_magic_method(op);

    let op_result = math_op(target_value, value_value, op_magic_method, arena)?;

    arena.update(&target_name.id, op_result);//.set(target_name.id.to_string(), op_result);
...
fn eval_constant(constant: &Constant, _range: &TextRange, arena: &mut PyArena) -> Result<PyObject, PyException> {
    match constant {
        ...
        Constant::Int(int_value) => {
            ...
            Ok(arena.statics.get_int(int_value.clone()))  // I don't like this clone
        },
